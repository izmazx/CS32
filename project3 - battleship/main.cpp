...
{
    return  g.addShip(1, 'A', "aircraft carrier")  &&
            g.addShip(4, 'S', "aircraft carrier")  &&
            g.addShip(3, 'B', "battleship")  &&
            g.addShip(2, 'D', "destroyer") ;
}
...
int main()
{
    ...
    else if (line[0] == '2')
    {
        Game g(5, 5);
        addStandardShips(g);
        Player* p1 = createPlayer("human", "lily", g);
        Player* p2 = createPlayer("human", "izma da boss", g);
        g.play(p1, p2);
        delete p1;
        delete p2;
    }
    else if (line[0] == '3')
    {
        ...
        for (int k = 1; k <= NTRIALS; k++)
        {
            ...
            Game g(10, 10);
            addStandardShips(g);
            Player* p1 = createPlayer("good", "slay Audrey", g);
